...
public class Player : MonoBehaviour
{
    ...
    Transform firePositionRoot;   // 트랜스폼을 여러개 가지는 배열([]가 배열)
    GameObject flash;
    ...
    IEnumerator Fire()
    {
        ...
        while (true)
        {
            for (int i = 0; i < firePositionRoot.childCount; i++)
            {
                ...
                GameObject bulletInstance = Instantiate(bullet, firePositionRoot.GetChild(i).position, firePositionRoot.GetChild(i).rotation);
