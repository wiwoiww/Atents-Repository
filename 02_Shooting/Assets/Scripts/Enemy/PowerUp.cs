...
{
    public float dirChangeTime = 1.0f;          // 파워업 아이템의 이동방향이 바뀌는데 걸리는시간
    ...
    private void Start()
    {
        ...
        player = FindObjectOfType<Player>();           // Player 타입을 찾기(무조건 한개만 찾기 때문에 Player가 여러개가 있을 경우 어떤 것이 들어올지는 알 수 없다.)
    ...
    private void Update()
    {
        transform.Translate(moveSpeed * Time.deltaTime * dir); // 현재 이동방향으로 초당 moveSpeed만큼 이동하기
    }
    ...
    void SetRandomDir(bool allRandom = true)        // 디폴트 파라메터. 값을 지정하지 않으면 디폴트 값이 대신 들어간다.
    {
        if (allRandom)
        {
            ...
            dir = dir.normalized;  // 단위벡터로 수정
        }
        else
        {
                Vector2 playerToPowerUp = transform.position - player.transform.position;  // 플레이어 위치에서 파워업 아이템 위치로 가는 방향벡터 계산.
                playerToPowerUp = playerToPowerUp.normalized;   // 단위 벡터로 변경
