...
public class ImageNumber : MonoBehaviour
{
    ...
    private void Update()
    {
        if((int)currentNumber != Number)    // currentNumber가 Number와 같아졌는지 확인. 다를 때만 아래코드 실행
        {
            ...
            if (dir > 0)
            {
                currentNumber = Mathf.Min(currentNumber, Number);       // currentNumber의 방향이 증가일 때 목표인 Number를 넘친 경우 Number로 설정
            }
            else
            {
                currentNumber = Mathf.Max(currentNumber, Number);       // currentNumber의 방향이 감소일 때 목표인 Number밑으로 내려간 경우 Number로 설정
            }
