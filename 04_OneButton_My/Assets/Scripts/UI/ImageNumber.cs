...
public class ImageNumber : MonoBehaviour
{
    ...
    private void Update()
    {
        if ((int)currentNumber != Number)  // currentNumber가 Number와 같아졌는지 확인. 다를 때만 아래코드 실행
        {
            ...
            float speed = numberChangeSpeed/* * Mathf.Pow(10, digits.Count) */;  // 일단 고정 속도로
            currentNumber += dir * speed * Time.deltaTime;                  // 방향에 따라 초당 numberChangeSpeed만큼 currentNumber 변화
            if (dir > 0)
            {
