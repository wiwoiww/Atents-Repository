...
namespace _1일차_복습
{
    ...
    public class Character // Character 라는 클래스를 public으로 선언한다.
    {
        ...
        string[] nameArray = { "너굴맨", "개굴맨", "ㅁㅁㅁ", "ㄷㄷㄷ", "ㅋㅋㅋ" };// nameArray에 기본값 설정
        ...
        public int HP
        {
            ...
            private set // 이 프로퍼티에 값을 넣을 때 호출되는 부분.set에 private을 붙히면 쓰는 것은 나만 가능
            {
                hp = value;
                if(hp > maxHP)
                {
        ...
        public Character()
        {
            ...
            randomNumber %= 5; //randomNumber = randomNumber % 5; // 랜덤으로 고른숫자를 0~4로 변경
        ...
        {
            Console.WriteLine("-----------------------------");
            Console.WriteLine($"l 이름\t: {name}");
            Console.WriteLine($"l HP\t: {hp} / {maxHP}");
            Console.WriteLine($"l 힘\t: {strenth,2}");
            Console.WriteLine($"l 민첩\t: {dexterity,2}");
            Console.WriteLine($"l 체력\t: {intellegence,2}");
            Console.WriteLine("-----------------------------");
